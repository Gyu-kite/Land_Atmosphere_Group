...
!  if(file_exists) then
!      ! 디버깅 정보 추가
!      write(LVT_logunit,*) '[DEBUG] File exists, expected dimensions: ', lisdaobs(source)%nc, lisdaobs(source)%nr
!      write(LVT_logunit,*) '[DEBUG] Expected data size: ', lisdaobs(source)%nc * lisdaobs(source)%nr * 4, ' bytes'
!      write(LVT_logunit,*) '[DEBUG] Scaled obs flag: ', lisdaobs(source)%scal
!      write(LVT_logunit,*) '[DEBUG] Obs type: ', lisdaobs(source)%obstype
!           
!     ftn = 11
...
!     close(ftn)
  ...
  if(file_exists) then
     ...
     ftn = 11
     open(ftn,file=trim(fname), form='unformatted', iostat=ios)
     write(LVT_logunit,*) '[DEBUG] File open status: ', ios
     
     if(ios == 0) then
        read(ftn, iostat=ios) obsData_inp
        write(LVT_logunit,*) '[DEBUG] First read status: ', ios
        
        if(ios == 0) then
           write(LVT_logunit,*) '[DEBUG] First read successful, data range: ', &
                minval(obsData_inp), maxval(obsData_inp)
            
           if(lisdaobs(source)%scal.eq.1) then
              write(LVT_logunit,*) '[DEBUG] Attempting second read for scaled obs...'
              read(ftn, iostat=ios) obsData_inp
              write(LVT_logunit,*) '[DEBUG] Second read status: ', ios
              
              if(ios == 0) then
                 write(LVT_logunit,*) '[DEBUG] Second read successful, data range: ', &
                      minval(obsData_inp), maxval(obsData_inp)
              else
                 write(LVT_logunit,*) '[ERR] Second read failed with iostat: ', ios
              endif
           endif
        else
           write(LVT_logunit,*) '[ERR] First read failed with iostat: ', ios
           write(LVT_logunit,*) '[ERR] This usually means end-of-file or wrong dimensions'
        endif
     else
        write(LVT_logunit,*) '[ERR] Failed to open file with iostat: ', ios
     endif
     ...
     do r=1,lisdaobs(source)%nr
        ...
        enddo
     enddo
     print*, 'DEBUG: nc, nr = ', lisdaobs(source)%nc,lisdaobs(source)%nr
