...
! Run each nest separately 
       ...
       do n=1,LIS_rc%nnest
          if(LIS_timeToRunNest(n)) then 
             ...
             write(LIS_logunit,*) '[DEBUG-DA] Before LIS_readDAobservations'
             call flush(LIS_logunit)
             call LIS_readDAobservations(n)
             write(LIS_logunit,*) '[DEBUG-DA] After LIS_readDAobservations'
             call flush(LIS_logunit)

             write(LIS_logunit,*) '[DEBUG-DA] Before LIS_perturb_DAobservations'
             call flush(LIS_logunit)
             call LIS_perturb_DAobservations(n)
             write(LIS_logunit,*) '[DEBUG-DA] After LIS_perturb_DAobservations'
             call flush(LIS_logunit)

             write(LIS_logunit,*) '[DEBUG-DA] Before LIS_perturb_writerestart'
             call flush(LIS_logunit)
             call LIS_perturb_writerestart(n)
             write(LIS_logunit,*) '[DEBUG-DA] After LIS_perturb_writerestart'
             call flush(LIS_logunit)

             write(LIS_logunit,*) '[DEBUG-DA] Before LIS_dataassim_run'
             call flush(LIS_logunit)
