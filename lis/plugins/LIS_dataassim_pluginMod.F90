...
! !USES:
   ...
   use lnetf_Mod,            only : lnetf_init, lnetf_setup,                  &
                                   ...
                                   lnetf_multisource_increments,             &
                                   lnetf_multisource_update,                 &
                                   lnetf_multisource_diagnostics
#endif
!----------------------------------------------------------------------
...
!LNETF - multi-source simultaneous mode
   call registerdamscompincr(trim(LIS_lnetfId)//char(0),&
        lnetf_multisource_increments)
   call registerdamsapplyincr(trim(LIS_lnetfId)//char(0),&
        lnetf_multisource_update)
   call registerdamsoutput(trim(LIS_lnetfId)//char(0),&
        lnetf_multisource_diagnostics)
#endif
