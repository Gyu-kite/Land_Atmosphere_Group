...
!   using the Haversine formula with robust error handling.
!
...
!
!   IMPORTANT: Includes input validation to handle:
!   - NaN values (returns 999999.0 km to exclude from localization)
!   - Inf or extremely large values
!   - Out-of-range latitudes (|lat| > 90)
!   - Numerical precision issues (clamps 'a' to [0,1])
!
!   This prevents SIGFPE (arithmetic exception) that can occur when
!   observation coordinates contain invalid values.
!
! !REFERENCES:
...
!EOP
    ...
    ! Input validation: Check for NaN (NaN /= NaN is true)
    if (lon1 /= lon1 .or. lat1 /= lat1 .or. lon2 /= lon2 .or. lat2 /= lat2) then
       haversine_km = 999999.0  ! Return large distance to exclude invalid observations
       return
    endif

    ! Input validation: Check for Inf or extremely large values
    if (abs(lon1) > 1.0e30 .or. abs(lat1) > 1.0e30 .or. &
        abs(lon2) > 1.0e30 .or. abs(lat2) > 1.0e30) then
       haversine_km = 999999.0
       return
    endif

    ! Input validation: Check for valid latitude range [-90, 90]
    ...
    ! Clamp 'a' to [0.0, 1.0] to handle numerical precision issues
    ! This prevents sqrt(1.0 - a) from attempting sqrt of negative number
    if (a < 0.0) then
       a = 0.0
    else if (a > 1.0) then
       a = 1.0
    ...
    ! Final NaN check before sqrt
    if (a /= a) then
       haversine_km = 999999.0
       return
    endif

    ! Compute distance - now safe because a is guaranteed to be in [0,1]
