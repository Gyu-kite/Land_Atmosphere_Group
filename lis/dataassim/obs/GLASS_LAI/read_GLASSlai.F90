...
! HDF4 SDS 함수들 직접 선언 (LDT처럼)
external :: sfstart, sfend, sfn2index, sfselect, sfendacc, sfrdata
integer :: sfstart, sfend, sfn2index, sfselect, sfendacc, sfrdata
#endif
!
! !INPUT PARAMETERS: 
! 
...
!
! !DESCRIPTION: 
!  This subroutine reads the GLASS LAI file using HDF4 SDS functions
!
!EOP
  ...
  integer                 :: iret,c,r,c1,r1
  character*50            :: lai_name
  integer                 :: start_hdf(2), edges_hdf(2), stride_hdf(2)
  ...
  ! 파일 존재 여부 확인
  inquire(file=trim(fname), exist=file_exists_check)
  write(LIS_logunit,*) '[DEBUG] File exists check:', file_exists_check
  write(LIS_logunit,*) '[DEBUG] Filename:', trim(fname)
  
  if(.not.file_exists_check) then
     write(LIS_logunit,*) '[ERR] File does not exist:', trim(fname)
     return
  endif

  ! HDF4 SDS 함수 사용 (LDT 방식)
  write(LIS_logunit,*) '[DEBUG] Using HDF4 SDS functions instead of HDFEOS2'
  
  sd_id = sfstart(trim(fname), DFACC_READ)
  if (sd_id.eq.-1) then
     write(LIS_logunit,*) "[ERR] Failed to open hdf file with sfstart", fname
     return
  end if
  
  write(LIS_logunit,*) '[DEBUG] Successfully opened HDF file with sfstart, SD ID:', sd_id
  
  lai_name = "LAI"
  sds_index = sfn2index(sd_id, lai_name)
  if (sds_index.eq.-1) then
     write(LIS_logunit,*) "[ERR] Cannot find LAI variable in file"
     status = sfend(sd_id)
     return
  end if
  
  write(LIS_logunit,*) '[DEBUG] LAI variable index:', sds_index
  
  sds_id = sfselect(sd_id, sds_index)
  if (sds_id.eq.-1) then
     write(LIS_logunit,*) "[ERR] Cannot select LAI variable"
     status = sfend(sd_id)
     return
  end if
  
  write(LIS_logunit,*) '[DEBUG] LAI variable selected, SDS ID:', sds_id
  ...
  if(GLASSlai_struc(n)%source.eq."MODIS") then 
     ...
     status = sfrdata(sds_id, start_hdf, stride_hdf, edges_hdf, lai_raw_modis)
     
     write(LIS_logunit,*) '[DEBUG] SDS read status:', status
     write(LIS_logunit,*) '[DEBUG] Raw MODIS LAI min/max:', minval(lai_raw_modis), maxval(lai_raw_modis)
     write(LIS_logunit,*) '[DEBUG] Valid raw LAI count:', count(lai_raw_modis > 0 .and. lai_raw_modis /= 255)
     ...
     do r=1,GLASSlai_struc(n)%nr
        do c=1,GLASSlai_struc(n)%nc
           ...
           if(lai_raw_modis(c1,r1).gt.0.and.&
                lai_raw_modis(c1,r1).ne.255) then 
              lai_in(c+(r-1)*GLASSlai_struc(n)%nc) =&
                   lai_raw_modis(c1,r1)*0.1
     ...
     write(LIS_logunit,*) '[DEBUG] Processed LAI min/max:', minval(lai_in), maxval(lai_in)
     write(LIS_logunit,*) '[DEBUG] Valid processed LAI count:', count(lai_in > -9999.0)
  endif
  ...
  write(LIS_logunit,*) '[DEBUG] Final LAI min/max:', minval(laiobs_ip), maxval(laiobs_ip)
  write(LIS_logunit,*) '[DEBUG] Final valid LAI count:', count(laiobs_ip > -9999.0)
