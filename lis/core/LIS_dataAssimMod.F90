...
!EOP
    ...
    !----------------------------------------------------------------------
    ! Multi-source DA mode: combine all observations and do single analysis
    ! Supports: EnKF, LNETF
    !----------------------------------------------------------------------
    if(LIS_rc%multisource_da == 1 .and. LIS_rc%ndas > 1) then
       ! Check algorithm type - EnKF or LNETF
       if(trim(LIS_rc%daalg(1)).eq."EnKF") then
          write(LIS_logunit,*) '[INFO] Running multi-source DA (simultaneous mode) with EnKF'
          call computeMultisourceIncrements("EnKF"//char(0), n)
          call applyMultisourceIncrements("EnKF"//char(0), n)
       elseif(trim(LIS_rc%daalg(1)).eq."LNETF") then
          write(LIS_logunit,*) '[INFO] Running multi-source DA (simultaneous mode) with LNETF'
          call computeMultisourceIncrements("LNETF"//char(0), n)
          call applyMultisourceIncrements("LNETF"//char(0), n)
       else
          write(LIS_logunit,*) '[WARN] Multi-source DA only supports EnKF/LNETF, falling back to sequential mode'
          goto 100
       endif
    else
       !----------------------------------------------------------------------
       ! Sequential DA mode (original): process each DA instance separately
       !----------------------------------------------------------------------
100    continue
       do j=1,LIS_rc%ndas
...
!write bias restart
    ...
    !----------------------------------------------------------------------
    ! Multi-source DA mode: use combined diagnostics output
    !----------------------------------------------------------------------
    if(LIS_rc%multisource_da == 1 .and. LIS_rc%ndas > 1) then
       if(trim(LIS_rc%daalg(1)).eq."LNETF") then
          ! Call multi-source diagnostics for combined output
          call daoutputMultisource("LNETF"//char(0), n)
       else
          ! Fallback to sequential output for other algorithms
          do j=1,LIS_rc%ndas
             if(LIS_rc%daalg(j).ne."none") then
       ...
       !----------------------------------------------------------------------
       ! Sequential DA mode: output for each DA instance separately
       !----------------------------------------------------------------------
       do j=1,LIS_rc%ndas
          if(LIS_rc%daalg(j).ne."none") then
             call daoutput(trim(LIS_rc%daalg(j))//char(0),n,j)
