...
!    endif
    ...
    if ( rc == 1 ) then
        call register_dataentry(lis_moc_lsm_count, LIS_MOC_CHV, &
            lis_histdata(n)%head_lsm_list,&
            n, 1, ntiles,(/"m/s"/), 1, (/"-"/),1,1,1,&
            !n, 2, ntiles,(/"m s-1","s m-1"/), 1, (/"-"/),1,1,1,&
            !n, 3, ntiles,(/"m s-1","s m-1","-    "/), 1, (/"-"/),1,1,1,&
            model_patch=.true.)
    endif
...
!EOP
    ...
    elseif(unit.eq."m/s") then 
       cfunit = "m s-1"
    elseif(unit.eq."m s-1") then
       cfunit = "m s-1"
    !elseif(unit.eq."s/m") then
    !   cfunit = "s m-1"
    !elseif(unit.eq."s m-1") then
    !   cfunit = "s m-1"
    elseif(unit.eq."s/m") then 
       cfunit = "s m-1"
...
!EOP
    ...
    unit_status = .false.
    !write(LIS_logunit,*) '[DEBUG] ===== UNIT COMPARISON DEBUG ====='
    !write(LIS_logunit,*) '[DEBUG] Input unit = "', trim(unit), '"'
    !write(LIS_logunit,*) '[DEBUG] Input unit length = ', len_trim(unit)
    !write(LIS_logunit,*) '[DEBUG] dataEntry%units = "', trim(dataEntry%units), '"'
    !write(LIS_logunit,*) '[DEBUG] dataEntry%nunits = ', dataEntry%nunits
    !do i=1,dataEntry%nunits
    !   write(LIS_logunit,*) '[DEBUG] Testing unittypes(', i, ') = "', trim(dataEntry%unittypes(i)), '"'
    !   if(unit.eq.dataEntry%unittypes(i)) then
    !      write(LIS_logunit,*) '[DEBUG] *** MATCH FOUND ***'
    !      unit_status = .true.
    !      exit
    !   else
    !      write(LIS_logunit,*) '[DEBUG] No match for: "', trim(unit), '" vs "', trim(dataEntry%unittypes(i)), '"'
    !   endif
    !enddo
    !write(LIS_logunit,*) '[DEBUG] Final unit_status = ', unit_status
    !write(LIS_logunit,*) '[DEBUG] ===== END DEBUG ====='
    ...
    !write(LIS_logunit,*) '[DEBUG] unit_status check for: ', trim(dataEntry%standard_name)
    !write(LIS_logunit,*) '[DEBUG] unit_status = ', unit_status
    !write(LIS_logunit,*) '[DEBUG] dataEntry%units = "', trim(dataEntry%units), '"'
    !write(LIS_logunit,*) '[DEBUG] dataEntry%nunits = ', dataEntry%nunits
    !do i = 1,dataEntry%nunits
    !   write(LIS_logunit,*) '[DEBUG] dataEntry%unittypes(', i, ') = "', trim(dataEntry%unittypes(i)), '"'
    !enddo
...
!   
      ...
      ierr = -1
      ! 디버깅 출력 추가
      !write(LIS_logunit,*) '[DEBUG] Checking units for: ', trim(dataEntry%standard_name)
      !write(LIS_logunit,*) '[DEBUG] dataEntry%units = "', trim(dataEntry%units), '"'
      !write(LIS_logunit,*) '[DEBUG] dataEntry%nunits = ', dataEntry%nunits
      !do i = 1,dataEntry%nunits
      !   write(LIS_logunit,*) '[DEBUG] dataEntry%unittypes(', i, ') = "', trim(dataEntry%unittypes(i)), '"'
      ...
      !      ierr = i
      !      write(LIS_logunit,*) '[DEBUG] MATCH found at index: ', i
      !      exit
      !   endif
      !enddo
      do i = 1,dataEntry%nunits
