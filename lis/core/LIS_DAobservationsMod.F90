...
!EOP
    ...
    write(LIS_logunit,*) '[DEBUG-PERTURB] Entering LIS_perturb_DAobservations, ndas=', LIS_rc%ndas
    call flush(LIS_logunit)
    do k=1,LIS_rc%ndas
       write(LIS_logunit,*) '[DEBUG-PERTURB] k=', k, ' perturb_obs=', trim(LIS_rc%perturb_obs(k))
       call flush(LIS_logunit)
...
!          if(mod(curr_time,real(LIS_rc%pertobsInterval(k))).eq.0)then
          write(LIS_logunit,*) '[DEBUG-PERTURB] Getting Data Update Status...'
          call flush(LIS_logunit)
          ...
          write(LIS_logunit,*) '[DEBUG-PERTURB] data_status=', data_status
          call flush(LIS_logunit)
          if(data_status) then
             write(LIS_logunit,*) '[DEBUG-PERTURB] Calling perturbmethod...'
             call flush(LIS_logunit)
             call perturbmethod(trim(LIS_rc%perturb_obs(k))//char(0),3,n,k, &
                  LIS_OBS_State(n,k), LIS_OBS_Pert_State(n,k))
             write(LIS_logunit,*) '[DEBUG-PERTURB] perturbmethod completed'
             call flush(LIS_logunit)
       ...
       write(LIS_logunit,*) '[DEBUG-PERTURB] wobs(k)=', LIS_rc%wobs(k)
       call flush(LIS_logunit)
       if(LIS_rc%wobs(k).gt.0) then
          write(LIS_logunit,*) '[DEBUG-PERTURB] Calling writeDAObs...'
          call flush(LIS_logunit)
          call writeDAObs(trim(LIS_rc%daset(k))//char(0), n, k, &
               LIS_OBS_State(n,k))
          write(LIS_logunit,*) '[DEBUG-PERTURB] writeDAObs completed'
          call flush(LIS_logunit)
       endif

    enddo
    write(LIS_logunit,*) '[DEBUG-PERTURB] LIS_perturb_DAobservations completed'
    call flush(LIS_logunit)
