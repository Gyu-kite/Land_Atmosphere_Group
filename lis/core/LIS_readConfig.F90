...
! the minimum timestep among different model components. 
  ...
  ! Read multi-source DA mode (0=sequential, 1=simultaneous)
  call ESMF_ConfigGetAttribute(LIS_config,LIS_rc%multisource_da,&
       label="Multi-source data assimilation mode:",rc=rc)
  if(rc.ne.0) then
     LIS_rc%multisource_da = 0  ! default: sequential mode
     write(LIS_logunit,*) '[INFO] Multi-source data assimilation mode not specified, using default: 0 (sequential)'
  else
     write(LIS_logunit,*) '[INFO] Multi-source data assimilation mode:', LIS_rc%multisource_da
     if(LIS_rc%multisource_da == 1) then
        write(LIS_logunit,*) '[INFO] Simultaneous multi-source DA enabled'
     endif
  endif
