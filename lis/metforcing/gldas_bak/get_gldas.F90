...
!-------------------------------------------------------------------
  ...
  write(UNIT=temp,FMT='(i4,i2,i2,a1,a4,a8)') uyr,umo,uda,'.',initcode, &
                                          '.021.nc4'
  read(UNIT=temp,FMT='(21A1)') ftime
  do i=1,8
     if(ftime(i).eq.(' ')) ftime(i)='0'
  enddo
  
  write(UNIT=temp,FMT='(A18)') 'GLDAS_NOAH025_3H.A'
  ...
  write(UNIT=temp,FMT='(200a1)') (fbase(i),i=1,c),'/', &
       (fsubs(i),i=1,18),(ftime(i),i=1,21)
  read(UNIT=temp, FMT='(a200)') name
  return
