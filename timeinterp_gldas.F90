...
!  25 Jan  2019: Sanghee Jun: rain rate, snow rate
! 
! !INTERFACE:
...
!EOP
  ...
  write(LIS_logunit,*) "its = -6*60*60", inittime, idoy, igmt, iyr ,ida, &
  ihr, imn, iss, its, gmt1, gmt2
  ...
  do t = 1, LIS_rc%ntiles(n)
     ...
     psurf(t) = (wt1 * gldas_struc(n)%metdata1(6,index1) & 
          + wt2 *gldas_struc(n)%metdata2(6,index1))
     wind_speed = wt1 * gldas_struc(n)%metdata1(5,index1) & 
          + wt2 * gldas_struc(n)%metdata2(5,index1)
