...
!  01 Jan  2019: Sanghee Jun: file name
! !INTERFACE:
...
! !DESCRIPTION:
!   This subroutine puts together ERA5 file name for 
...
!   hour of day
!  \item[ncold]
!   Number of columns (used as to determine the resolution)
!  \end{description}
!
!EOP
  integer uyr,umo,uda,uhr,i,c
  character(len=4) :: initcode
  character*1 fbase(80)
...
!  ERA5 uses 4-digit hour format: 0000, 0300, 0600, etc.
!-------------------------------------------------------------------
  if(uhr<3)then
     initcode = '0000'   
  elseif(uhr<6)then
  ...
  write(UNIT=temp,FMT='(A)') trim(gldasdir)
  ...
  ! Create the full filename in ERA5_MSWEP format
  ! Format: ERA5_MSWEP_025_3h.YYYYMMDD.HHMM.nc
  write(UNIT=temp,FMT='(A,A,A,I4.4,I2.2,I2.2,A,A,A)') &
       trim(gldasdir), '/', 'ERA5_MSWEP_025_3h.', &
       uyr, umo, uda, '.', initcode, '.nc'
  
  read(UNIT=temp, FMT='(A)') name
