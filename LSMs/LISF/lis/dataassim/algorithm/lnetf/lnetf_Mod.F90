...
!----------------------------------------------------------------------------
     do n=1,LIS_rc%nnest
        ...
        if(status.ne.0) then 
           ...
           write(LIS_logunit,*) '[INFO] LNETF localization radius factor not found in config, using default: 5.0'
        else
...
! 
! !DESCRIPTION: 
!  This routine computes the analysis increments for LNETF. The state variable
...
!   \item[assemble\_obs\_cov](\ref{assemble_obs_cov_lnetf})
...
!    modeling point.
!   \item[lnetf\_analysis](\ref{lnetf_analysis}) \newline
!    apply the LNETF filter to obtain the prognostic variable
!    increments. 
!   \item[row\_variance](\ref{row_variance_lnetf}) \newline
...
!----------------------------------------------------------------------------
    ...
    if(data_status) then  
       write(LIS_logunit,*) &
            '[INFO] Assimilating Observations using LNETF for DA instance',k
